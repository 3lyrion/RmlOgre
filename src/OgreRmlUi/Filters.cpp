...
{
    auto* originalCmd = renderInterface.getLastDrawCommand();
    assert(originalCmd);
    DrawCommand cmd = *originalCmd;
    renderInterface.injectNewRenderable(cmd, material);
    renderInterface.addDrawCommand(cmd);
}
...
void SingleMaterialFilter::release(RenderInterface& renderInterface)
{
	...
	for(auto* textureUnit : pass->getTextureUnitStateIterator())
	{
		...
		renderInterface.releaseTexture(texture);
	}
}
...
void BlurFilter::apply(RenderInterface& renderInterface)
{
	...
	if (this->halfSamples > 0)
	{
		cmd.scissor.x *= 0.50f;
		cmd.scissor.y *= 0.50f;
        renderInterface.injectNewRenderable(cmd, halfsample);
        renderInterface.addDrawCommand(cmd);
		...
		for(int i = 0; i < this->halfSamples - 1; ++i)
		{
		    cmd.scissor.x *= 0.50f;
		    cmd.scissor.y *= 0.50f;
			cmd.scissor.z = (cmd.scissor.z + 1.0f) * 0.50f;
			cmd.scissor.w = (cmd.scissor.w + 1.0f) * 0.50f;
            renderInterface.injectNewRenderable(cmd, halfsample);
            renderInterface.addDrawCommand(cmd);
		}
	}

	cmd.scissor.x = originalCmd->scissor.x;
	cmd.scissor.z = originalCmd->scissor.z;
	cmd.scissor.w = originalCmd->scissor.w;

    renderInterface.injectNewRenderable(cmd, blurH);
    renderInterface.addDrawCommand(cmd);

    renderInterface.injectNewRenderable(cmd, blurV);
    renderInterface.addDrawCommand(cmd);
}
		...
		*shadowMaterial = *this->shadowMaterial;
		shadowMaterial->load();
		{
			auto* pass = shadowMaterial
				...
				->getPass(0);
			auto vertexProgramParameters = pass->getVertexProgramParameters();
			vertexProgramParameters->setNamedConstant("offset", util::toOgre(offset));
			auto fragmentProgramParameters = pass->getFragmentProgramParameters();
			fragmentProgramParameters->setNamedConstant("colour", util::toOgre(colour));
		}
		...
		*shadowMaterial = *this->blurlessShadowMaterial;
		shadowMaterial->load();
		{
			auto* pass = shadowMaterial
				...
				->getPass(0);
			auto fragmentProgramParameters = pass->getFragmentProgramParameters();
			fragmentProgramParameters->setNamedConstant("offset", util::toOgre(offset));
			fragmentProgramParameters->setNamedConstant("colour", util::toOgre(colour));
		}
