...
Ogre::TextureGpu* RenderInterface::acquireLayerTexture(size_t textureId, uint vpWidth, uint vpHeight, Ogre::TextureGpu& mainRTT)
{
    ...
    if (texture)
    {
        assert(texture->isRenderToTexture());
        return texture;
    }
...
void RenderInterface::drawIntoCompositor(Ogre::RenderPassDescriptor* renderPassDesc, Ogre::TextureGpu* anyTargetTexture, Ogre::Camera const* currentCamera, Ogre::RenderTargetViewDef const* rtv)
{
    ...
    if (numNeededDraws > 0)
    {
        ...
        for (size_t i = 0; i < numNeededDraws; ++i)
        {
            ...
            if (!renderable)
            {
                ...
                Rml::Mesh mesh;
                Rml::MeshUtilities::GenerateQuad(mesh, { 0.0f, 0.0f }, size, { 255, 255, 255, 255 });
    ...
    for (size_t i = 0; i < numNeededDraws; i++)
    {
        ...
        case DrawCommand::Type::SaveLayerAsTexture:
        {
            ...
            bool restartPass = (currentPassDesc == layer.passDesc);
            if (restartPass)
                renderSystem->endRenderPassDescriptor();
...
void RenderInterface::injectNewRenderable(DrawCommand& command, Ogre::MaterialPtr const& material)
{
    ...
    renderable.setMaterial(command.textureId ? m_baseMaterial : m_blankMaterial);
}
